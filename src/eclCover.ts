...
import { getEclRuntimePath } from './eclClientTools';
...
export function getCoverage(filename: string): Promise<any[]> {
	return new Promise((resolve, reject) => {
		let tmppath = path.normalize(path.join(os.tmpdir(), 'ecl-code-cover'));
		let cwd = path.dirname(filename);
		let args = ['test', '-coverprofile=' + tmppath];
		let eclRuntimePath = getEclRuntimePath();
		...
		cp.execFile(eclRuntimePath, args, { cwd: cwd }, (err, stdout, stderr) => {
			try {
				// Clear existing coverage files
				clearCoverage();
				if (err && (<any>err).code === 'ENOENT') {
					vscode.window.showInformationMessage('Could not generate coverage report.  Install ECL from http://hpccsystems.com/.');
					return resolve([]);
				}
				...
				lines.on('line', function (data: string) {
					// ecl test coverageprofile generates output:
